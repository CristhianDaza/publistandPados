...
const { catalogs, loading, fetchCatalogs, removeCatalog } = useCatalogs()
...
const columns = [
  ...
  { key: 'link', label: 'Enlace', type: 'link' },
...
const handleCreate = () => {
  router.push('/admin/catalogs/new')
}

const handleEdit = (item) => {
  router.push(`/admin/catalogs/${item.id}`)
}
...
const confirmDelete = async () => {
  ...
  try {
    ...
    await removeCatalog(itemToDelete.value.id)
...
<template>
  <div>
    ...
    <div v-if="loading && catalogs.length === 0" class="flex justify-center py-12">
    ...
    >
      <div class="bg-slate-900 border border-slate-800 rounded-xl p-6 w-full max-w-md shadow-2xl">
        <div class="flex items-center gap-4 mb-4">
          ...
          <div>
            <h3 class="text-lg font-bold text-white">¿Eliminar catálogo?</h3>
