<template>
  <div class="relative w-full h-[600px] overflow-hidden group">
    ...
    >
      ...
      >
        <div class="text-center px-4 max-w-4xl mx-auto transform transition-transform duration-1000 delay-300 pointer-events-auto"
          ...
          <h2 v-if="item.title" class="text-5xl md:text-7xl font-bold text-white mb-6 tracking-tight">
            {{ item.title }}
          </h2>
          <p v-if="item.description" class="text-xl md:text-2xl text-gray-200 mb-8 font-light">
            {{ item.description }}
          </p>
          <UButton
            v-if="item.link"
            ...
            color="white"
            variant="solid"
            class="px-8 py-3 rounded-full font-semibold hover:scale-105 transition-transform"
          >
            Ver más
          </UButton>
        </div>
    ...
    <button
      @click="prevSlide"
      class="absolute left-4 top-1/2 -translate-y-1/2 z-20 w-12 h-12 flex items-center justify-center rounded-full bg-white/10 backdrop-blur-md text-white hover:bg-white/20 transition-all opacity-0 group-hover:opacity-100"
    >
      <UIcon name="i-heroicons-chevron-left" class="w-8 h-8" />
    </button>
    <button
      @click="nextSlide"
      class="absolute right-4 top-1/2 -translate-y-1/2 z-20 w-12 h-12 flex items-center justify-center rounded-full bg-white/10 backdrop-blur-md text-white hover:bg-white/20 transition-all opacity-0 group-hover:opacity-100"
    >
