  ...
  <section class="py-24 relative bg-background overflow-hidden">
    ...
    <UContainer>
      <div class="text-center mb-16 relative z-10">
        <h2 class="text-3xl md:text-4xl font-bold mb-4 text-primary animate-gradient-x pb-2">
          {{ title }}
        </h2>
      </div>

      <div class="relative grid gap-8 md:grid-cols-3">
        <div class="hidden md:block absolute top-12 left-[16%] right-[16%] h-0.5 bg-gradient-to-r from-transparent via-secondary/20 to-transparent -z-10 border-t-2 border-dashed border-secondary/30" />
        ...
        >
          <div class="h-full p-8 rounded-2xl bg-background/40 backdrop-blur-md border border-secondary/20 shadow-xl hover:shadow-2xl transition-all duration-300 hover:-translate-y-1">
            <div class="mb-6 relative inline-block">
              <div class="w-16 h-16 rounded-2xl bg-primary flex items-center justify-center shadow-lg transform group-hover:scale-110 transition-transform duration-300">
                <UIcon :name="step.icon" class="w-8 h-8 text-white" />
              </div>
              <div class="absolute -top-3 -right-3 w-8 h-8 rounded-full bg-background border-2 border-primary flex items-center justify-center font-bold text-primary shadow-sm">
                {{ index + 1 }}
              </div>
            </div>
